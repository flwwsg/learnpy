...
class FtpTools:
	...
	def getpassword(self):
		return getpass('Password for %s on %s:' % 
	...
	def uploadOne(self, localname, localpath, remotename):
		if self.isTextKind(localname):
			...
			localfile = open(localpath, 'rb')
			self.connection.strobinary('STOR '+remotename, localfile)
		localfile.close()
...
if __name__=='__main__':
	...
	if xfermode == 'download':
		ftp.configTransfer()
		ftp.run(cleanTarget=ftp.clenaLocals, transferAct=ftp.downloadDir)
